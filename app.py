...
# TỐI ƯU HÓA UI BẰNG CSS KHÔNG GIAN, MOBILE & HIỆU ỨNG
st.markdown("""
<style>
    /* 1. Xóa khoảng trống thừa trên cùng của Streamlit */
    .block-container {
    ...
    /* 2. Giấu các UI không cần thiết */
    div[data-testid="InputInstructions"] { display: none !important; }
    header {visibility: hidden;} /* Giấu thanh header trắng mặc định */
    footer {visibility: hidden;} /* Giấu footer */
    
    /* 3. Tối ưu nút bấm (Nổi bật, bo góc, gradient, hiệu ứng nảy nhẹ khi ấn) */
    button[kind="primary"] {
...
if not st.session_state.logged_in:
    st.markdown("<h2 style='text-align: center; margin-bottom: 1rem;'><span class='wiggle-icon'>🔐</span> Đăng nhập Share Bills</h2>", unsafe_allow_html=True)
    ...
    if user_data and user_data.get('app_data'):
        ...
        if is_old_format:
            ...
            for old_name, info in raw_members.items():
                if old_name == st.session_state.nickname: new_id = my_id
                else: new_id = f"user_{int(time.time()*1000)}_{len(new_members)}"
...
st.markdown("<h2 style='margin-bottom: 0px;'><span class='wiggle-icon'>💸</span> Share Bills Ultimate V7</h2>", unsafe_allow_html=True)
tab1, tab2, tab3, tab4, tab5 = st.tabs(["👥 Danh Bạ & Nhóm", "🧾 Ghi Hóa Đơn", "🔥 Chốt Sổ Nợ", "🕒 Nhật Ký Bill", "🎁 Thống Kê"])

# --- TAB 1: DANH BẠ & NHÓM ---
with tab1:
    st.markdown(f"#### 👤 Thông tin cá nhân")
    bank_list = ["", "MB", "VCB", "TPB", "BIDV", "TCB", "VPB", "CTG", "ACB", "SHB", "STB", "VIB"]
    with st.expander("⚙️ Sửa thông tin của bạn", expanded=False):
        c0, c1, c2 = st.columns([1.5,1,1.5])
    ...
    with c_add:
        with st.container(border=True):
            ...
            c_bk, c_ac = st.columns([1, 1.5])
            new_f_bank = c_bk.selectbox("Bank:", bank_list, key="new_f_bank")
            new_f_acc = c_ac.text_input("STK:", key="new_f_acc")
            if st.button("Thêm bạn", type="primary", use_container_width=True):
                if not new_f_name.strip(): st.warning("Tên không được trống!")
                else:
    ...
    with c_list:
        ...
        if friend_ids:
            st.markdown("**📜 Danh bạ (Tùy chỉnh cá nhân)**")
            for f_id in friend_ids:
                ...
                with st.expander(f"👤 {f_data['name']}"):
                    ...
                    edit_f_name = fc0.text_input("Tên:", value=f_data['name'], key=f"en_{f_id}", label_visibility="collapsed")
                    edit_f_bank = fc1.selectbox("Bank:", bank_list, index=bank_list.index(f_bank_saved) if f_bank_saved in bank_list else 0, key=f"eb_{f_id}", label_visibility="collapsed")
                    edit_f_acc = fc2.text_input("STK:", value=f_data.get('acc', ''), key=f"ea_{f_id}", label_visibility="collapsed")
                    ...
                    if bc1.button("Lưu", key=f"sb_{f_id}", use_container_width=True):
                        ...
                        save_data(); st.rerun()
                    if bc2.button("Xóa", key=f"db_{f_id}", type="primary", use_container_width=True):
        ...
        if st.session_state.groups:
            ...
            for g_name, g_members in list(st.session_state.groups.items()):
                with st.expander(f"📌 {g_name} ({len(g_members)} người)"):
                    ...
                    if gc1.button("Lưu Nhóm", key=f"sg_{g_name}", use_container_width=True) and len(edit_g_members) >= 2:
                        st.session_state.groups[g_name] = edit_g_members; save_data(); st.rerun()
                    if gc2.button("Xóa Nhóm", key=f"dg_{g_name}", type="primary", use_container_width=True):
...
with tab2:
    ...
    st.markdown("#### 🤖 Đọc Bill Nhanh bằng AI")
    c_ai1, c_ai2 = st.columns(2)
    current_time_str = datetime.now(timezone(timedelta(hours=7))).strftime('%d/%m/%Y %H:%M')
    
    with c_ai1:
        up_files = st.file_uploader("📸 Nhập bill ở đây", type=["jpg", "png", "jpeg"], accept_multiple_files=True, label_visibility="collapsed")
        if up_files and st.button("✨ Phân tích ảnh", type="primary", use_container_width=True):
    ...
    with c_ai2:
        txt_ai = st.text_area("💬 Hoặc dán tin nhắn chữ:", height=70, label_visibility="collapsed", placeholder="Ví dụ: Lẩu 500k, bò thêm 150k")
        if txt_ai and st.button("✨ Phân tích chữ", type="primary", use_container_width=True):
    ...
    st.divider()
    st.markdown("#### 📝 Nhập món thủ công")
    ic1, ic2, ic3, ic4 = st.columns([4, 3, 2, 2])
    im_n = ic1.text_input("Tên món:")
    im_p = ic2.text_input("Giá (VNĐ):")
    im_q = ic3.number_input("SL", 1, 100, 1)
    with ic4:
        st.markdown("<div class='mobile-margin-fix' style='margin-top: 28.5px;'></div>", unsafe_allow_html=True)
    ...
    if st.session_state.current_items:
        ...
        for idx, it in enumerate(st.session_state.current_items):
            ...
            if c_d.button("❌", key=f"d_{idx}", use_container_width=True): st.session_state.current_items.pop(idx); st.rerun()

    st.divider()
    c_info1, c_info2 = st.columns(2)
    b_title = c_info1.text_input("Tiêu đề cuộc vui:", value="Đi ăn")
    b_date = c_info2.text_input("Thời gian:", value=st.session_state.get("ai_date", datetime.now().strftime("%d/%m/%Y %H:%M")))
    if total_bill == 0: total_bill = parse_amount(st.text_input("💰 Nhập tổng bill nhanh (Nếu không cần chi tiết):", "0"))

    st.divider()
    st.markdown(f"#### 💳 Ai đã quẹt thẻ? (Tổng: <span style='color:#ff4b4b'>{format_vn(total_bill)}đ</span>)", unsafe_allow_html=True)
    all_mem_ids = list(st.session_state.members.keys())
    selected_payers = st.multiselect("Người ứng tiền:", all_mem_ids, default=[my_id], format_func=get_name, label_visibility="collapsed")
    ...
    st.divider()
    st.markdown("#### 🍴 Nhóm nào đi ăn?")
    use_g = st.selectbox("Chọn nhóm nhanh:", ["-- Chọn lẻ --"] + list(st.session_state.groups.keys()), label_visibility="collapsed")
    def_m = st.session_state.groups.get(use_g, all_mem_ids) if use_g != "-- Chọn lẻ --" else all_mem_ids
    b_cons = st.multiselect("Chọn từng người:", def_m, default=def_m, format_func=get_name, label_visibility="collapsed")
    ...
    if b_cons and total_bill > 0:
        ...
        if method == "Chia theo món lẻ" and st.session_state.current_items:
            pool = 0
            for idx, it in enumerate(st.session_state.current_items):
                who = st.multiselect(f"🍴 **{it['name']}** ({format_vn(it['price']*it['qty'])}đ) - Ai ăn?", b_cons, default=b_cons, format_func=get_pure_name, key=f"w_{idx}")
                if who:
        ...
        has_deadline = st.checkbox("📅 Có hạn chót thanh toán không?")
        b_deadline = st.date_input("Chọn ngày:", value=datetime.now().date()).strftime("%d/%m/%Y") if has_deadline else None

        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("💾 CHỐT ĐƠN & LƯU SỔ", type="primary", use_container_width=True):
            if not selected_payers: st.error("Chưa chọn người trả tiền!")
                ...
                save_data(); st.balloons(); st.success("🎉 Đã lưu thành công!"); time.sleep(1.5); st.rerun()
...
with tab3:
    ...
    if not unpaid: 
        st.markdown("<h3 style='text-align: center; color: #4CAF50; padding: 30px 0;'>Tất cả hóa đơn đã được thanh toán! 🎉</h3>", unsafe_allow_html=True)
    else:
        # ==========================================
        # MỤC 1: CÔNG TẮC BÙ TRỪ
        # ==========================================
        use_netting = st.toggle("🔀 Bật tính toán bù trừ nợ chéo tự động", value=True)
        ...
        msg_raw = "📣 TỔNG KẾT CHỐT SỔ (Chưa bù trừ):\n"
        if debts_dict:
        ...
        else: msg_raw += "Không có khoản nợ nào.\n"

        msg_netting = "📣 TỔNG KẾT CHỐT SỔ (Đã bù trừ thông minh):\n"
        ...
        else: msg_netting += "Không có khoản nợ nào cần chốt.\n"
        ...
        # ==========================================
        st.divider()
        st.markdown("**📋 Copy tin nhắn gửi đòi nợ:**")
        if use_netting: st.code(msg_netting, language="text")
        else: st.code(msg_raw, language="text")
        ...
        if not use_netting:
            ...
            st.markdown("#### 📜 Danh sách nợ chéo thông minh")
            if not found_netting: 
                st.info("Tuyệt vời, không có khoản nợ nào cần xử lý.")
            else:
                for i in range(len(list(st.session_state.members.keys()))):
                    for j in range(i + 1, len(list(st.session_state.members.keys()))):
                        m1, m2 = list(st.session_state.members.keys())[i], list(st.session_state.members.keys())[j]
                        if matrix[m1][m2] > matrix[m2][m1]:
                            ...
                            if net_amt > 1:
                                with st.expander(f"👉 **{get_pure_name(m1)}** cần chuyển **{get_pure_name(m2)}**: {format_vn(net_amt)}đ"):
                                    ...
                                    if st.button(f"✅ Xác nhận đã chuyển khoản", key=f"n_{m1}_{m2}", type="primary"):
                            ...
                            if net_amt > 1:
                                with st.expander(f"👉 **{get_pure_name(m2)}** cần chuyển **{get_pure_name(m1)}**: {format_vn(net_amt)}đ"):
                                    ...
                                    if c_info.get('bank') and c_info.get('acc'):
                                        st.image(f"https://img.vietqr.io/image/{c_info['bank']}-{c_info['acc']}-compact2.png?amount={int(net_amt)}", width=250)
                                    if st.button(f"✅ Xác nhận đã chuyển khoản", key=f"n_{m2}_{m1}", type="primary"):
...
with tab4:
    ...
    if not st.session_state.history: 
        st.markdown("<h3 style='text-align: center; color: #ff4b4b; padding: 50px 0;'>🙌 Chưa có hoá đơn nào, lên kèo đi chơi thôi!</h3>", unsafe_allow_html=True)
    else:
        filter_col1, filter_col2 = st.columns(2)
        status_filter = filter_col1.selectbox("Trạng thái:", ["Tất cả", "🔴 Đang nợ", "✅ Đã thanh toán xong"])
        sort_order = filter_col2.radio("Sắp xếp:", ["Mới nhất", "Cũ nhất"], horizontal=True)
        st.divider()
        ...
        if sort_order == "Mới nhất": display_history = list(reversed(display_history))
        
        for real_idx, b in display_history:
            status_txt = " (Xong ✅)" if b.get('status') == 'paid' else ""
            with st.expander(f"[{b['date']}] {b['name']} - {format_vn(b['amount'])}đ {status_txt}"):
                ...
                st.markdown(f"**Nguồn tiền:** {p_str}")
                for p_id, amt in b['splits'].items():
                    if amt > 0:
                        is_done = "✅" if p_id in b.get('paid_by', []) or b.get('status') == 'paid' else "🔴"
                        st.write(f"- {get_pure_name(p_id)} ăn: {format_vn(amt)}đ {is_done}")
                
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button(f"🗑️ Xóa bill này", key=f"del_b_{b['id']}", type="primary", use_container_width=True): confirm_delete_bill(real_idx, b['name'])

# --- TAB 5: WRAPPED & ANALYTICS ---
with tab5:
    my_id = st.session_state.username
    my_name = get_pure_name(my_id)
    st.markdown(f"<h3 style='text-align: center; color: #ff4b4b;'>🎉 Share Bills Wrapped - {my_name}</h3>", unsafe_allow_html=True)
    ...
    if not st.session_state.history:
        ...
        if not filtered_history: 
            ...
            for b in filtered_history:
                ...
                if my_id in splits: my_spent += splits[my_id]
                            ...
                            for c_id, c_bal in pos_bals.items():
                                ...
                                if owed_amt > 1000: 
                                    all_unpaid_debts.append({
                                        ...
                                        "amount": int(owed_amt), "item": b.get('name', 'Bill')
                                    })

                gn = "Nhóm chung"
                for g_name, g_members in st.session_state.groups.items():
                    if set(splits.keys()) == set(g_members): gn = g_name; break
            ...
            c1.metric("Tiền ăn bạn đã chi", f"{format_vn(my_spent)}đ")
            ...
            if m_debtor_id: c2.metric("Nợ bạn nhiều nhất", get_pure_name(m_debtor_id), f"{format_vn(others_owe_me.get(m_debtor_id, 0))}đ / Tổng: {format_vn(tot_o_me)}đ", delta_color="normal")
            else: c2.metric("Nợ bạn", "Không ai", "0đ", delta_color="off")
            
            m_creditor_id = max(i_owe_others, key=i_owe_others.get) if i_owe_others else None
            if m_creditor_id: c3.metric("Bạn nợ nhiều nhất", get_pure_name(m_creditor_id), f"-{format_vn(i_owe_others.get(m_creditor_id, 0))}đ / Tổng: -{format_vn(tot_i_o)}đ", delta_color="inverse")
            else: c3.metric("Bạn nợ", "Không ai", "0đ", delta_color="off")

            st.divider()
            st.markdown("#### 🔥 Xếp Hạng Đóng Góp Nhóm")
            for i, (name, s) in enumerate(sorted(group_stats.items(), key=lambda x: x[1]['count'], reverse=True)):
                medal = "🥇" if i == 0 else "🥈" if i == 1 else "🥉" if i == 2 else "🔹"
                st.write(f"{medal} **{name}**: {s['count']} kèo - Quy mô: {format_vn(s['money'])}đ")

            st.divider()
            st.markdown("#### 📊 Top 10 Khoản Nợ Hệ Thống")
            if all_unpaid_debts:
